...
export const contarLogs = async ({ usuario_id, acao, data_inicio, data_fim }) => {
  const conditions = [];
  const params = [];

  if (usuario_id) { conditions.push('usuario_id = ?'); params.push(usuario_id); }
  if (acao) { conditions.push('acao_realizada LIKE ?'); params.push(`%${acao}%`); }
  if (data_inicio) { conditions.push('data_hora >= ?'); params.push(data_inicio); }
  if (data_fim) { conditions.push('data_hora <= ?'); params.push(data_fim + ' 23:59:59'); }

  const where = conditions.length > 0 ? 'WHERE ' + conditions.join(' AND ') : '';
  const sql = `SELECT COUNT(*) AS total FROM logs_sistema ${where}`;
  const [results] = await pool.query(sql, params);
